use std::{collections::{HashMap, VecDeque}, future::Future, task::Waker, sync::{Mutex, Arc}, ops::{DerefMut, Deref}};

use anyhow::{Error, anyhow};
use axum::{Router, routing::get, extract::{Query, State}, http::StatusCode, response::{IntoResponse, Response}};
...
impl Future for &ReqPoll {
    ...
    fn poll(self: std::pin::Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> std::task::Poll<Self::Output> {
        ...
        match std::mem::take(data.deref_mut()) {
            ...
            None => { *self.waker.lock().expect("") = Some(cx.waker().clone()); std::task::Poll::Pending }
        }
    }
}
...
async fn notify(Query(mut params): Query<HashMap<String, String>>, State(futures): State<Futures>) -> StatusCode {
    let Some(token) = params.remove("token") else {
        return StatusCode::BAD_REQUEST
    };
    let suspended = {
        let mut guard = futures.lock().expect("");
        let suspended: Vec<_> = guard.deref().into_iter().filter(|entry| entry.0 == token ).map(|(_, r)| r.clone()).collect();
...
#[debug_handler]
async fn poll_notified(Query(NotifyWait{token}): Query<NotifyWait>, State(futures): State<Futures>) -> (StatusCode, Result<String, AppError>) {
    //FIXME: Limit futures
    let p = Arc::new(ReqPoll::new());
    {
        let mut guard = futures.lock().expect("");
        if guard.deref().len() > MAX_FUTURES {
            guard.deref_mut().pop_front().unwrap().1.fulfill(Err(anyhow!("You got kicked")))
        }
        guard.deref_mut().push_back((token, p.clone()));
    }
    let data = p.as_ref().await;
    let Ok(data) = data else {
        return (StatusCode::REQUEST_TIMEOUT, Err(AppError(data.err().unwrap())))
    };    
    (StatusCode::OK, serde_json::to_string(&data).map_err(|e| AppError(anyhow!(e.to_string()))))    
}